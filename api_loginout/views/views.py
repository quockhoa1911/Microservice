...
class LoginoutViews(ModelViewSet):
        ...
        try:
            ...
            if account_role.exists():
                ...
                multi_thread = Multi_Thread(html=string_html,target=Send_Mail_Service.send_mail,
                                            content_main_body='Content_main_body', header='Welcome to Shopbee', from_email='',
    ...
    def forgot_password(self,request,*args,**kwargs):
        ...
        account = Accounts.objects.filter(Q(username=username) & Q(profile__email=email) & Q(profile__certificate=certificate))
        if account.exists():
            account = account.first()
