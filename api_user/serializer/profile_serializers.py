...
class Profile_serializers(serializers.ModelSerializer):
    ...
    class Meta:
        model = Profiles
        fields = ['id', 'name', 'gender', 'age', 'phone_number', 'email', 'certificate', 'avatar',
                  'profile_payment_type', 'province',
...
class Profile_write_serializers(serializers.ModelSerializer):
    ...
    def to_internal_value(self, data):  # hook point before on going to validate data
